...
app.get('/', function(req, res) {
	res.sendFile(path.join(client_path, 'index_code_snippet.html'));
});
...
// add a websocket service to the http(s) server:
	...
	ws.on('message', function(msg) {
		if (msg instanceof Buffer) {
			...
			if (msg == "getData") {
				...
				console.log("hi")
        ws.send(JSON.stringify({ cmd:"newData", state: "testing state" }))
...
server.listen(8080, function() {
	...
	console.log(`client view on http://localhost:${server.address().port}/index_code_snippet.html\n\n`);
});